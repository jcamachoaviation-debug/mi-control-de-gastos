...
# Estilos corregidos: Eliminamos el fondo forzado para evitar conflictos de contraste
st.markdown("""
    <style>
    /* Estilizado de tarjetas manteniendo la adaptabilidad del color de fuente del tema */
...
with st.sidebar:
    ...
    with st.expander("➕ Registrar Operacion", expanded=True):
...
# Filtro Temporal en la parte superior (Layout Gerencial)
c1, c2 = st.columns(2)
...
# --- SECCIÓN 1: KPIs GERENCIALES (Contenedores Nativos para evitar fallos de contraste) ---
...
with col_g1:
    # Gráfico de Dona: Distribución de Gastos (Con paleta adaptativa y contraste asegurado)
    ...
    if not df_gst.empty:
        ...
        # Asegurar que el fondo del gráfico sea transparente para heredar el tema de Streamlit
        fig_donut.update_layout(
            margin=dict(t=40, b=40, l=20, r=20),
            paper_bgcolor='rgba(0,0,0,0)',
            plot_bgcolor='rgba(0,0,0,0)',
            font=dict(color='#888888' if gst == 0 else None) # Deja que el tema controle si no hay error
        )
...
# --- SECCIÓN 2: CONTROL DE PRESUPUESTOS (BARRAS DE PROGRESO ADAPTATIVAS) ---
...
for i, cat in enumerate(CATEGORIAS_GASTO):
    ...
    with cols_alert[i % 4]:
        ...
        # El texto de soporte normatizado para alto contraste
        if percent >= 1.0:
...
with st.expander("🔍 Auditoria de Movimientos"):
    if not df.empty:
        ...
        st.info("Sin registros indexados para el periodo actual.")
